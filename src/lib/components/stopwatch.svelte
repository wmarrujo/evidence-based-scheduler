<script lang="ts">
	...
	let spent = $derived($stopwatch.time ?? 0)
</script>
...
<div class="flex items-center gap-2" title={$stopwatch.task?.name ?? ""}>
	...
	{#if $stopwatch.task}
		{#if $stopwatch.running}
			<Button onclick={() => stopwatch.pause()} variant="outline" class="aspect-square p-2">
				<Pause />
			</Button>
		{:else}
			<Button onclick={() => stopwatch.resume()} variant="outline" class="aspect-square p-2">
				<Play />
			</Button>
		{/if}
