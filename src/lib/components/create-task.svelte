	...
	<div class="grid grid-cols-3 gap-2 mt-2 content-start">
		...
		<Form.Field {form} name="estimate" class="flex flex-col">
			<Form.Control let:attrs>
				...
				<Input type="number" bind:value={$data.estimate} min="0" step="0.1" {...attrs} class="w-full text-right" />
			</Form.Control>
		</Form.Field>
		<Form.Field {form} name="spent" class="flex flex-col" >
			<Form.Control let:attrs>
				<Form.Label>Spent</Form.Label>
				<Input type="number" bind:value={$data.spent} min="0" step="0.1" {...attrs} class="w-full text-right" />
			</Form.Control>
		</Form.Field>
